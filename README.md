## Hi, I'm Rajesh 👋

🚀 Power BI | Fabric | SQL | Azure

I am a Microsoft Certified Power BI Architect & Fabric Specialist with 20+ years of experience delivering enterprise-scale BI solutions across Finance, Healthcare and Retail. Expert in Power BI, Microsoft Fabric, Databricks, Azure Data Factory, DAX, and Lakehouse Architecture. Trusted advisor to executives — translating complex data into clear business outcomes.

---

## 📊 Projects

### 🔹 Sales Analytics Dashboard
- Built using Power BI + Fabric Lakehouse
- Implemented Incremental Refresh
- Applied Dynamic RLS

### 🔹 Financial Reporting Model
- Star schema design
- Advanced DAX measures
- Calculation Groups for Time Intelligence

---

## 🛠 Skills
**Analytics & Visualization**
Power BI | Microsoft Fabric | SSRS | DAX | M | Power Query | DAX Studio

**Cloud & Data Engineering**
Azure Data Factory | Databricks | PySpark | Delta Lake | Lakehouse Architecture | Azure Synapse | Snowflake

**Programming & Scripting**
Python | T-SQL | SparkSQL | Oracle PL/SQL | PowerShell

**Architecture & Governance**
Medallion Architecture | RLS & Data Security | CI/CD | Azure DevOps | Git | Deployment Pipelines | SOX | HIPAA | GDPR

**Other Tools**
Power Apps | Power Automate | SSIS | SQL Server | Excel | Access

## 🏆 Certifications

✅ Microsoft Certified Fabric Developer — DP-600
✅ Microsoft Certified Power BI Developer — PL-300
✅ Databricks Certified Data Engineer Professional
✅ Azure Solutions Architect Expert — AZ-305
✅ Azure Administrator Associate
✅ Microsoft Certified Technology Specialist

---
🔗 [Executive KPI Dashboard](https://github.com/rajeshnadendla/Executive-KPI-Dashboard)


## 📫 Contact
- LinkedIn: https://linkedin.com/in/rbnadendla
- Email: nv.rajeshbabu@gmail.com
